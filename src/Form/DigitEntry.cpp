...
void
DigitEntry::CreateRadioFrequency(ContainerWindow &parent, const PixelRect &rc,
                                 const WindowStyle style)
{
...
void
DigitEntry::SetValue(RadioFrequency value)
{
  assert(length == 4);
...
DigitEntry::GetRadioFrequency() const
{
  ...
  unsigned mega_hertz = 118 + columns[0].value;
  unsigned kilo_hertz = columns[2].value * 100 + columns[3].value * 5;

  return RadioFrequency::FromMegaKiloHertz(mega_hertz, kilo_hertz);
}
...
DigitEntry::OnPaint(Canvas &canvas) noexcept
{
  ...
  for (unsigned i = 0; i < length; ++i) {
    ...
    case Column::Type::RADIO_FREQURENCY_MHZ_OFFSET:
      assert(c.value < 19);
      StringFormat(buffer, sizeof(buffer), "%03u", c.value + 118);
      break;

    case Column::Type::RADIO_FREQURENCY_KHZ_SCALED:
      assert(c.value < 20);
      StringFormat(buffer, sizeof(buffer), "%02u", c.value * 5);
      break;
